%data=[0.01*rand(10000,14); 0.01+0.99*rand(1372,14)];
x = c301_x;
trainx = x;
trainx(:,13:14) = [];
maxepoch = epoch_n(1);
...
%%
new_data = 1./(1 + exp(-trainx*vishid - repmat(hidbiases,size(trainx,1),1)));
new_data = mapminmax(new_data')';
new_data = (new_data > rand(size(new_data,1),size(new_data,2)))*1;
